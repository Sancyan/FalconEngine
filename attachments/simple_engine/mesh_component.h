...
struct InstanceData {
  ...
  static vk::VertexInputBindingDescription getBindingDescription() {
    constexpr vk::VertexInputBindingDescription bindingDescription(
      1,
      // binding (binding 1 for instance data)
      sizeof(InstanceData),
      // stride
...
class MeshComponent final : public Component {
  ...
  public:
    ...
    void UpdateInstance(size_t index, const glm::mat4& transform, uint32_t materialIndex = 0) {
      assert(index < instances.size() && "UpdateInstance: index out of bounds");
    ...
    [[nodiscard]] const InstanceData& GetInstance(size_t index) const {
      assert(index < instances.size() && "GetInstance: index out of bounds");
      assert(!instances.empty() && "GetInstance: instances vector is empty");
      if (index < instances.size()) {
        return instances[index];
      }
      // Fallback to first instance if assertion is disabled
